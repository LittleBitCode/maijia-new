...
$(function(){
    ...
    $("body").on("click", ".one_button", function () {
        ...
        $.ajax({
            type: "POST",
            url: "/trade/refund_step2_1_submit/<?php echo $trade_info->id; ?>",
            data: {
                "goods_url": goods_url,
    ...
    $(".two_but_box").click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "/trade/refund_step2_2_submit/<?php echo $trade_info->id; ?>",
            data: {
                "is_post":is_post
            },
            datatype: "json",
    ...
    $('.trade_box').on('click','.next_step',function(){
        $.ajax({
            type: "POST",
            url: "/trade/refund_step2_3_submit/<?php echo $trade_info->id; ?>",
            data: {
                "a":1
            },
            datatype: "json",
...
function setImagePreview4(obj) {
    ...
    //验证平台展示图
    $(".tmall_goods_pic_con2").children(".error").remove();
    //验证图片格式
    if(!/\.(gif|jpg|jpeg|png|GIF|JPG|PNG)$/.test(docObj.files[0]['name'])){
        toastr.warning("图片格式必须是gif，jpg，png中的一种");
        return false;
