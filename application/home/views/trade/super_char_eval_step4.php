...
<div class="trade_top">
    <div class="Process">
        <ul class="clearfix">
            ...
            <li style="width:16%"><em class="Processyes">4</em><span>选择活动数量</span></li>
    ...
    <div class="step3_box">
        ...
        <table class="table table-bordered text-center">
            ...
            <?php if (count($app_scans) > 0): ?>
                <tr>
                    <td><span class="red">超级浏览任务</span></td>
                    <td><span class="red"><?php echo SUPER_SCAN_PRICE; ?></span>金币/个</td>
                    <td><span class="red"><?php echo count($app_scans); ?></span> x <span class="sd_number red"> <?php echo $trade_info->total_num; ?></span>单</td>
                    <td><span class="scan_fee_total red"><?php echo bcmul(bcmul(count($app_scans), SUPER_SCAN_PRICE,2), $trade_info->total_num, 2); ?></span>金币</td>
                </tr>
            <?php endif; ?>
        </table>
    </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $(".next_step").click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "/trade/super_char_eval_step4_submit/" + _trade_id,
                data: {
                    "total_num": total_num,
    ...
    function fee_total() {
        ...
        var scan_num = <?php echo count($app_scans); ?>;
        //单数
        ...
        $(".scan_fee_total").text((parseFloat(scan_num) * parseFloat(sd_num) * parseFloat(<?php echo SUPER_SCAN_PRICE ?>)).toFixed(2) )
        // 如果是手机端活动执行下面订单分布计算
        var task_type = 'phone';
        if (task_type == 'phone') {
