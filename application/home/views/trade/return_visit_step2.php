...
<body>
            ...
            <div class="one_box">
                ...
                <div class="search_type_box white_box">
                    ...
                    <div class="phone_search_box">
                        <label>
                            <input type="checkbox" name="phone_check" <?php if ($trade_info->is_phone): ?>checked<?php endif; ?> />手机淘宝活动<span>（用户"手机淘宝活动"搜索下单）</span>
                        </label>
                        <div class="phone_taobao_con">
                            ...
                            <div class="phone_keyword_list_box">
                                ...
                                <div class="phone_keyword_list">
                                    <label><span class="color_red">* </span>手机淘宝关键字来源<em><?php echo $k+1; ?></em>：<?php if ($k > 0): ?><a href="javascript:;" class="phone_keyword_list_del">删除</a></label><?php endif; ?></label>
                                    <div class="keyword_search">
                                        <input class="blue_input" type="text" name="app_keyword[]" value="<?php echo $v->kwd; ?>" maxlength="50"/><span class="color_red">(必填)</span>.
                                    </div>
    ...
    <div class="popup_wrap one_button_publishing" style="display: none;">
        <div class="publishing_wrap">
            <p class="publishing_remind">为了避免活动审核不通过，影响您的活动效率，请先核实好以下信息是否和您的淘宝信息保持一致，并且确保您提供的关键字，可以在前5页搜索到您的商品，活动一旦审核通过，不得更改淘宝商品信息，否则撤销对应活动。</p>
...
$(function(){
    ...
    $(".phone_keyword_add a").click(function(){
        ...
        $(".phone_keyword_list_box").append('<div class="phone_keyword_list">'+
            '<label>手机淘宝关键字来源<em>'+keyword_num+'</em>：<a href="javascript:;" class="phone_keyword_list_del">删除</a></label>'+
            '<p class="keyword_search">让买手打开<em>手机淘宝活动</em>搜索关键字进入"搜索列表"<input type="text" name="app_keyword[]" maxlength="50"/><em>必填</em></p>'+
            '<div class="price_range">价格区间：<input type="text" name="price_start" onblur="javascript:this.value=this.value.replace(\/[^\\d.]\/g,\'\')" onkeyup="this.value=this.value.replace(\/[^\\d.]\/g,\'\')" />元&nbsp;-<input type="text" name="price_end" onblur="javascript:this.value=this.value.replace(\/[^\\d.]\/g,\'\')" onkeyup="this.value=this.value.replace(\/[^\\d.]\/g,\'\')" />元</div>'+
            '<div class="service">'+
                '<span>折扣和服务：</span>'+
                '<p>'+
            ...
            '<div class="row"><div class="col-xs-2"><div class="address">'+
                ...
                '</span>'+
            '</div></div>'+
            '<div class="classification col-xs-9"><p>商品分类：</p><input type="text" name="goods_cate[]" class="blue_input" /></div>'+
        '</div></div>');
        if (keyword_num > 4) {
    ...
    $("body").on("click", ".one_button", function () {
        ...
        if( !$('input[name="pc_check"]').is(":checked") && !$('input[name="phone_check"]').is(":checked") ){
            toastr.warning('请选择使用"淘宝搜索框"查找商品或者使用"手机淘宝"查找商品');
            return false;
        }
        ...
        if ($(".phone_search_box>label>input").is(":checked")) {
            ...
            var search_html = '<p style="padding-left:20px">使用"手机淘宝搜索框"查找商品</p>';
            var i = 1;
        ...
        $.ajax({
            type: "POST",
            url: "/trade/return_visit_step2_1_submit/" + _trade_id,
            data: {
                "goods_url": goods_url,
    ...
    $(".two_but_box").click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "/trade/return_visit_step2_2_submit/" + _trade_id,
    ...
    $('.trade_box').on('click','.next_step',function(){
        ...
        $.ajax({
            type: "POST",
            url: "/trade/return_visit_step2_3_submit/" + _trade_id,
            data: {"a": 1},
            datatype: "json",
            success: function(d) {
